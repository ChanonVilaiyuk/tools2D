...
class minifig2D() : 
	...
	def mayaUI(self) : 
		...
		mc.window(self.ui, t = 'Monkey 2D Tool v.1.2')
		mc.columnLayout(adj = 1,rs = 3)

		# mc.iconTextButton( style='iconOnly', image1='O:/systemTool/python/tools2D/ApesCaesar.jpg', label='')
		mc.text(l = 'Project')
		...
		mc.frameLayout(borderStyle = 'etchedIn', lv = False)
		mc.rowColumnLayout(nc = 4, cw = ([1, 150], [2, 100], [3, 100], [4, 100]), cs = ([2, 2]))

		mc.button(l = '3D cam <--Switch--> 2D cam', h = 30, bgc = [0.8, 0.8, 0.3], c = partial(self.switchCamera))
		mc.button(l = 'Key 2D PanZoom', h = 30, bgc = [1, 0.2, 0.2], c = partial(self.setKey))
		mc.button(l = '+ Light', h = 30, bgc = [1, 1, 1], c = partial(self.createLight))
		mc.button(l = 'BlackHole', h = 30, bgc = [0.2, 0.2, 0.2], c = partial(self.assignBlackhole))
		...
		mc.button(l = 'Delete Selected .bat', bgc = [1, 1, 1], c = partial(self.removeFile))
		...
		mc.button(l = 'Refresh', c = partial(self.listRenderLayer), bgc = [1, 1, 1])
		...
		mc.showWindow()
		mc.window(self.ui, e = True, wh = [460, 606])
		...
		if frames : 
			...
			for eachFrame in frames : 
				...
				if rigGrp and camera and eachFrame : 
					...
					if value == '%s_exportAllRDB' % self.ui : 
						self.exportThisSceneCmd(frame, camera)
						self.listBatFile()
	...
	def exportThisSceneCmd(self, frame, camera) : 

		namespace = mc.textField('%s_cameraTX' % self.ui, q = True, tx = True)
		namespace = namespace.replace('_cam', '')
		...
		currentFile = mc.file(save = True)
		result = fileUtils.copy(currentFile, fileName)

		if result : 
			self.genBatchFile(frame, camera, mayaRenderPath, fileName)

		else : 
			mm.eval('warning "Copy Failed";')
	...
	def removeFile(self, arg = None) : 
		...
		for each in sels : 
			batFile = self.fileInfo[each]['batFile'].replace('/', '\\')
			mayaFile = batFile.replace('.bat', '.ma')
			
			try : 
				fileUtils.remove(batFile)
				print 'Remove %s' % batFile

			except : 
				pass

			try : 
				fileUtils.remove(mayaFile)
				print 'Remove %s' % mayaFile
